...
void WITextEntry::Initialize()
{
	...
	pBase->SetAutoAlignToParent(true);
	pBase->AddCallback("OnTextEntered",FunctionCallback<>::Create(std::bind([](WIHandle hTextEntry) {
		...
		te->CallCallbacks<void>("OnTextEntered");
	},this->GetHandle())));
...
std::string WITextEntry::GetText()
{
	if(!m_hBase.IsValid())
		return "";
...
void WITextEntry::GetSelectionBounds(int *start,int *end)
{
	if(!m_hBase.IsValid())
	{
		*start = 0;
...
void WINumericEntry::Initialize()
{
	...
	if(!m_numeric.hUpArrow.IsValid())
	{
		...
		m_numeric.hUpArrow->AddCallback("OnMousePressed",FunctionCallback<>::Create([this,hThis]() {
			if(!hThis.IsValid())
				return;
			auto text = GetText();
	...
	if(!m_numeric.hDownArrow.IsValid())
	{
		...
		auto hThis = GetHandle();
		m_numeric.hDownArrow->AddCallback("OnMousePressed",FunctionCallback<>::Create([this,hThis]() {
			if(!hThis.IsValid())
				return;
			auto text = GetText();
			auto i = atoi(text.c_str());
